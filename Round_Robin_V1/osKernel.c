...
{
		tcbs[i].stackpt = &TCB_STACK[i][STACK_SIZE-16];//pt to top of stack for particular thread
...
//we want this to happen in critical section so it needs to be atomic and then so we need to disable interrupt	
	...
	//below we are initializing thread for task 0
	  osKernalStackInit(0);//
	  TCB_STACK[0][STACK_SIZE-2]=(int32_t)(task0);//as we know this is location of PC and then pt to the thread0 so we could start executing it...
