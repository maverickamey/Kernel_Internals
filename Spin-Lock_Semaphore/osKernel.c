...
{
	SysTick->VAL = 0; //val keeps current count value of systick as next time with new interrupt occurs then here the value which was stored here from there it will start
	//current count value of systick
	//bring address of interrupt ctrl register
	INTCTRL = 0x04000000;//setting bit 26th trigger systick
	
}	

void osSemaphoreInit(int32_t *semaphore,int32_t value)    //just like we discuss washroom examples of 3 keys here value is the keys which is assigned to semaphore count for eg 3 keys =3=semaphore
{
		*semaphore  = value;
}	
